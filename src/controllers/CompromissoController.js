...
class CompromissoController {
  ...
  async postCompromissoAll(req, res) {
    try {
      ...
      const result = await CompromissoService.postComprissoAll({
        usuario_id,
        titulo,
        descricao,
        dataCompromisso,
      ...
      const response = ResponseModel(
        404,
        null,
        MessageModel.postFail("compromisso")
      );
      res.status(404).send(response);
    }
  }
  ...
  async postCompromisso(req, res) {
    try {
      const { usuario_id } = req.userData;
      const { titulo, descricao, dataCompromisso, horario, classificacao } =
        req.body;
  ...
  async putCompromisso(req, res) {
    try {
      ...
      const response = ResponseModel(
        404,
        null,
        MessageModel.putFail("compromisso")
      );
      res.status(404).send(response);
    }
  }
  ...
  async deleteCompromisso(req, res) {
    try {
      const { compromisso_id } = req.params;
      const result = await CompromissoService.deleteCompromisso(compromisso_id);
      const response = ResponseModel(
        200,
        result,
      ...
      const response = ResponseModel(
        404,
        null,
        MessageModel.deleteFail("compromisso")
      );
      res.status(404).send(response);
    }
  }
